...
local async_check_navigate_history = function (...)
    local test_scout, expected_index, expected_entry = ...

    assert.equals(test_scout.searchbar_manager:get_searchbar_contents(), expected_entry)
    assert.equals(test_scout.history.viewing_index, expected_index)
end

local async_check_only_searches = function (...)
    local test_scout, search_string, expected_count = ...

    assert.equals(test_scout.searchbar_manager:get_searchbar_contents(), search_string)
    assert.equals(#test_scout.history.entries, expected_count)

    if expected_count > 0 then
        assert.equals(test_scout.history:get_entry(expected_count), search_string)
    end
end
...
describe('Functional: History', function ()
    ...
    it('can navigate user\'s search history', function ()
        ...
        utils:async_asserts(consts.test.async_delay, async_check_navigate_history, scout, 1, "define")

        utils:keycodes_user_keypress(def_keymaps.next_history)
        utils:async_asserts(consts.test.async_delay, async_check_navigate_history, scout, 2, "->")

        utils:keycodes_user_keypress(def_keymaps.next_history)
        utils:async_asserts(consts.test.async_delay, async_check_navigate_history, scout, 3, "int")

        utils:keycodes_user_keypress(def_keymaps.next_history)
        utils:async_asserts(consts.test.async_delay, async_check_navigate_history, scout, 4, "const")

        utils:keycodes_user_keypress(def_keymaps.next_history)
        utils:async_asserts(consts.test.async_delay, async_check_navigate_history, scout, 5, "node *")

        utils:keycodes_user_keypress(def_keymaps.next_history)
        utils:async_asserts(consts.test.async_delay, async_check_navigate_history, scout, 6, "state")

        utils:keycodes_user_keypress(def_keymaps.next_history)
        utils:async_asserts(consts.test.async_delay, async_check_navigate_history, scout, 7, "create_")
    ...
    it('only adds entries that are actively searched for', function ()
        ...
        utils:async_asserts(consts.test.async_delay, async_check_only_searches, scout, search_text, 0)
        ...
        utils:async_asserts(consts.test.async_delay, async_check_only_searches, scout, search_text, 1)
    end)
