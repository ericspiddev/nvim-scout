...
function scout_search_mode:new(mode_name, mode_symbol, ns, text_color, border_hl)
    local obj = {
        name = mode_name,
        symbol = mode_symbol,
        search_bar_win = Scout_Consts.window.INVALID_WINDOW_ID,
        display_col = 0,
        banner_config = {
            relative='win',
            row=2,
            zindex=1,
            width=#mode_name + Scout_Consts.modes.padding_space,
            height=1,
            border = {},
            focusable=false,
            footer={{Scout_Consts.modes.footer_text, Scout_Consts.colorscheme_groups.m_virt_text_c}},
            style="minimal",
        },
        active = false,
        namespace = ns,
...
function scout_search_mode:get_banner_display_width()
    return  #self.name + Scout_Consts.modes.padding_space
end
...
function scout_search_mode:get_extra_padding()
    ...
    if has_bg_color then
        return Scout_Consts.modes.padding_space + 1
    else
        return Scout_Consts.modes.padding_space
    end
end
