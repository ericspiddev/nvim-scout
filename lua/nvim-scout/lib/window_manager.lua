...
function scout_window_manager:get_managed_window(name)
    ...
    vim.print("Could not find window with name " .. vim.inspect(name))
end
...
function scout_window_manager:get_window_field(name, field, validator)
    return self:perform_window_action(name, function (window)
        if validator then
            if validator(window[field]) then
                return window[field]
            else
                Scout_Logger:error_print("Get field failed passed in validator for field: ", field)
            end
        else
            return window[field]
        end
    ...
    return self:perform_window_action(name, function (window)
        if validator then
            if validator(value) then
                ...
                Scout_Logger:error_print("Set field failed passed in validator for field: ", field)
            end
        else
            window[field] = value
        end
...
function scout_window_manager:get_window_host(name)
    return self:get_window_field(name, "host", vim.api.nvim_win_is_valid)
end
